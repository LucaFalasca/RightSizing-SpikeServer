...
Variando il carico di lavoro da 1 req/s a 12 req/s e impostando \(SI_{max} = \infty\), in modo da misurare solo il web server, si può verificare che il throughput del sistema non superi mai il suo limite teorico, che nel caso considerato è uguale a \(min(\mu_{web}, \lambda_{web}) \). Tuttavia dalla figura \ref{fig:wrong_throughput} si nota come il throughput misurato superi il valore teorico per carichi di lavoro elevati. Andando ad investigare il problema, il problema era dovuto dalla gestione di fine simulazione. Infatti, siccome alla fine della simulazione le code non vengono svuotate ci sono tutta una serie di job ancora in coda che non vengono considerati nella metrica di throughput, calcolata come \( num\_completed / sim\_time \). 
...
Se invece calcoliamo il throughput a partire dall'utilizzazione come \( X = U * \mu \), otteniamo i risultati mostrati in figura \ref{fig:correct_throughput}, dove si può notare come il throughput misurato sia sempre inferiore al valore teorico.
...
Come spiegato nelle sezioni precedenti, si è scelto di non svuotare le code per rimanere fedeli al caso di studio di riferimento, dove questo comportamento viene gestito in questo modo.