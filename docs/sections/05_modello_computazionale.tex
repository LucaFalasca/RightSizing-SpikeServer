...
\begin{itemize}
    \item Se \(\mathrm{len}(web\_jobs) < SI_{max}\), il job viene aggiunto a web\_jobs.
...
\begin{itemize}
    ...
    \item Stream 2: Servizio Spike Server (Iperesponenziale, media 0.16s, cv=4).
\end{itemize}
...
\end{lstlisting}
È importante notare che per ogni campionamento dell'esponenziale si utilizzano, quindi, due estrazioni dallo stream del generatore di numeri casuali. 
...
\begin{itemize}
    ...
    \item N\_PROCESSES: numero di processi paralleli per l'esecuzione delle repliche (40 processi)
\end{itemize}
...
\begin{itemize}
    \item ad ogni processo vengono assegnati 3 stream dei 256 disponibili, in modo che ogni processo abbia il proprio spazio di numeri casuali indipendenti dagli altri processi, ma in maniera tale che possano procedere in parallelo senza che la non deterministicità del parallelismo influisse sull'ordine di estrazione dei numeri casuali.
    \item Per il numero di repliche introdotte, ogni processo non si avvicinava nemmeno lontanamente al limite dello stream assegnato. \[MODULUS / STREAM \approx 8388607\] È stato comunque implementato un meccanismo in cui una volta sforato il limite dello stream, il processo riceva 3 nuovi stream esclusivi. Ed essendo il numero di processi 40, e ogni processo usa 3 stream, questo è tranquillamente possibile senza sforare il limite dei 256 stream e senza ulteriori meccanismi di controllo.
    \item Sempre per garantire la riproducibilità dei risultati, ad ogni processo vengono assegnate sempre lo stesso numero di repliche da eseguire, in modo tale che le ultime esecuzioni non vengano assegnate a processi diversi (e quindi stream diversi) in base all'ordine di esecuzione di essi.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{utilization\_web}: utilizzo medio del web server \[ \frac{area\_busy\_web}{TEMPO\_FINE\_SIMULAZIONE - BIAS\_PHASE} \]
    \item \textbf{utilization\_spike}: utilizzo medio dello spike server \[ \frac{area\_busy\_spike}{TEMPO\_FINE\_SIMULAZIONE - BIAS\_PHASE} \] 
    \item \textbf{throughput\_web}: throughput medio del web server \[ \frac{completed\_web}{TEMPO\_FINE\_SIMULAZIONE - BIAS\_PHASE} \]
    \item \textbf{throughput\_spike}: throughput medio dello spike server \[ \frac{completed\_spike}{TEMPO\_FINE\_SIMULAZIONE - BIAS\_PHASE} \]
    \item \textbf{throughput\_total}: throughput medio totale \[ \frac{completed\_web + completed\_spike}{TEMPO\_FINE\_SIMULAZIONE - BIAS\_PHASE} \]
